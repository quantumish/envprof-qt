...
void populate_env(QGroupBox* box, Func* main)
{
...
void populate_targets(QGroupBox* targets, std::vector<Func*> expensive, uint64_t total)
{
	...
	for (Func* f : expensive) {
		...
		QString str;
		double percent = f->energy/static_cast<double>(total);
...
int main(int argc, char *argv[])
{
	...
	populate_targets(targets, expensive, prof.total);	
	// populate_env(env, expensive[0]);
